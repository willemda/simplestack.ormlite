...
namespace SimpleStack.OrmLite.DDLTest
{
	[TestFixture()]
	public class Test
	{
		private List<Dialect> dialects = new List<Dialect>();
			...
			dialects.Add(new Dialect
				             {
					             ...
					             AddFKString =
						             "ALTER TABLE Child ADD CONSTRAINT JustOneFK FOREIGN KEY (IdModel) REFERENCES Model (Id) ON DELETE CASCADE ON UPDATE NO ACTION;",
					             AddFKRestrictString =
						             "ALTER TABLE Child ADD CONSTRAINT JustOneMoreFK FOREIGN KEY (IdModel) REFERENCES Model (Id) ON UPDATE NO ACTION;",
			...
			dialects.Add(new Dialect
				             {
					             ...
					             AddFKString =
						             "ALTER TABLE `Child` ADD CONSTRAINT `JustOneFK` FOREIGN KEY (`IdModel`) REFERENCES `Model` (`Id`) ON DELETE CASCADE ON UPDATE NO ACTION;",
					             AddFKRestrictString =
						             "ALTER TABLE `Child` ADD CONSTRAINT `JustOneMoreFK` FOREIGN KEY (`IdModel`) REFERENCES `Model` (`Id`) ON DELETE RESTRICT ON UPDATE NO ACTION;",
					             CreateIndexString = "CREATE UNIQUE INDEX `JustIndexOnColumn3` ON `Model`(`Column3`);"
				             });
			...
			foreach (var d in dialects)
			{
				...
				Assert.AreEqual(d.AddFKString,
				                d.Provider.ToAddForeignKeyStatement<Child, Model>(f => f.IdModel,
				                                                                  fk => fk.Id, OnFkOption.NoAction,
				                                                                  OnFkOption.Cascade, "JustOneFK"));
			}
		}
		...
		public void CanAddForeignKeyRestrict()
		{
			foreach (var d in dialects)
			{
				...
				Assert.AreEqual(d.AddFKRestrictString,
				                d.Provider.ToAddForeignKeyStatement<Child, Model>(f => f.IdModel,
				                                                                  fk => fk.Id, OnFkOption.NoAction,
				                                                                  OnFkOption.Restrict, "JustOneMoreFK"));
			}
		}
		...
		public void CanCreateIndex()
		{
			...
			{
				OrmLiteConfig.DialectProvider = d.Provider;
				Assert.AreEqual(d.CreateIndexString,
				                d.Provider.ToCreateIndexStatement<Model>(f => f.Column3, "JustIndexOnColumn3", true));
			}
		}
	}
