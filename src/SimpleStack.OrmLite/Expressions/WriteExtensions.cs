...
namespace SimpleStack.OrmLite
{
	public static class WriteExtensions
	{
		...
		/// </summary>
		public static int UpdateOnly<T>(this IDbCommand dbCmd, T model,
		...
		public static int Update<T>(this IDbCommand dbCmd, object updateOnly, Expression<Func<T, bool>> where = null)
		{
			...
			foreach (var setField in updateOnly.GetType().GetPublicProperties())
			{
				var fieldDef = fields.FirstOrDefault(x =>
				                                     string.Equals(x.Name, setField.Name,
		...
		/// </summary>
		public static void InsertOnly<T>(this IDbCommand dbCmd, T obj,
		                                 Func<SqlExpressionVisitor<T>, SqlExpressionVisitor<T>> onlyFields)
		{
