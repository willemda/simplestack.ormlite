...
namespace SimpleStack.OrmLite
{
		...
		where TDialect : IOrmLiteDialectProvider
	{
		...
		public virtual string ToInsertRowStatement(IDbCommand command, object objWithProperties,
		...
		public virtual IDbCommand CreateParameterizedInsertStatement(IDbConnection connection, object objWithProperties,
		                                                             ICollection<string> insertFields = null)
		{
			if (insertFields == null)
				insertFields = new List<string>();
		...
		public void ReParameterizeInsertStatement(IDbCommand command, object objWithProperties,
		                                          ICollection<string> insertFields = null)
		{
			if (insertFields == null)
				insertFields = new List<string>();
		...
		protected virtual void AddParameterForFieldToCommand(IDbCommand command, FieldDefinition fieldDef,
		...
		public virtual IDbCommand CreateParameterizedUpdateStatement(IDbConnection connection, object objWithProperties,
		                                                             ICollection<string> updateFields = null)
		{
			...
			foreach (var fieldDef in modelDef.FieldDefinitions)
			{
				...
				try
				{
					if (fieldDef.IsPrimaryKey && updateFields.Count == 0)
					{
						...
						sqlFilter.AppendFormat("{0} = {1}", GetQuotedColumnName(fieldDef.FieldName),
						                       String.Concat(ParamString, fieldDef.FieldName));
						AddParameterForFieldToCommand(command, fieldDef, objWithProperties);
					...
					if (sql.Length > 0) sql.Append(",");
					sql.AppendFormat("{0} = {1}", GetQuotedColumnName(fieldDef.FieldName),
					                 String.Concat(ParamString, fieldDef.FieldName));
			...
			command.CommandText = string.Format("UPDATE {0} SET {1} {2}", GetQuotedTableName(modelDef), sql,
			                                    (sqlFilter.Length > 0 ? "WHERE " + sqlFilter : ""));
			return command;
		}
		                                                            ...
		                                                            string foreignKeyName = null)
		{
			...
			return string.Format("ALTER TABLE {0} ADD CONSTRAINT {1} FOREIGN KEY ({2}) REFERENCES {3} ({4}){5}{6};",
			                     ...
			                     GetForeignKeyOnDeleteClause(new ForeignKeyConstraint(typeof (T),
			                                                                          onDelete: FkOptionToString(onDelete))),
			                     GetForeignKeyOnUpdateClause(new ForeignKeyConstraint(typeof (T),
			                                                                          onUpdate: FkOptionToString(onUpdate))));
		}
