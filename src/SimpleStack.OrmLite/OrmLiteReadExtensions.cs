...
namespace SimpleStack.OrmLite
{
	...
	public static class OrmLiteReadExtensions
	{
		...
		internal static List<TModel> Select<TModel>(this IDbCommand dbCmd, Type fromTableType, string sqlFilter,
		...
		{
			return First<T>(dbCmd,
			                OrmLiteConfig.DialectProvider.GetQuotedColumnName(ModelDefinition<T>.PrimaryKeyName) +
			                " = {0}".SqlFormat(idValue));
		}

		[ThreadStatic] private static Type lastQueryType;
		...
		internal static List<T> GetByIds<T>(this IDbCommand dbCmd, IEnumerable idValues)
		{
			...
			return sql == null
				       ? new List<T>()
				       : Select<T>(dbCmd,
				                   OrmLiteConfig.DialectProvider.GetQuotedColumnName(ModelDefinition<T>.PrimaryKeyName) + " IN (" +
				                   sql + ")");
		}
