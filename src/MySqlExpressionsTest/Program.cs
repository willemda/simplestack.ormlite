...
namespace MySqlExpressionsTest
{
	public class Author
	{
		public Author()
	...
	internal class MainClass
	{
		public static void Main(string[] args)
		{
				...
				"Server = 127.0.0.1; Database = ormlite; Uid = root; Pwd = password".OpenDbConnection())
			{
				...
				result = db.Select(ev);
				author = result.FirstOrDefault();
				Console.WriteLine("Expected:{0} ; Selected:{1}, OK? {2}", "Jorge Garzon".ToUpper(), author.Name,
				                  "Jorge Garzon".ToUpper() == author.Name);

				ev.Limit(8, 4); // third page
				result = db.Select(ev);
				author = result.FirstOrDefault();
				Console.WriteLine("Expected:{0} ; Selected:{1}, OK? {2}", "Rodger Contreras".ToUpper(), author.Name,
				                  "Rodger Contreras".ToUpper() == author.Name);
