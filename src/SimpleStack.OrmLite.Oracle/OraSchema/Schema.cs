	...
	public class Schema : ISchema<Table, Column, Procedure, Parameter>
	{
		...
		public List<Column> GetColumns(string tableName)
		{
			string sql = string.Format(sqlColumns.ToString(),
			                           string.IsNullOrEmpty(tableName) ? "\'\'" : string.Format("\'{0}\'", tableName));
			...
			{
				record.IsPrimaryKey =
					(Connection.GetScalar<int>(string.Format(sqlColConstrains.ToString(), tableName, record.Name, "P")) > 0);
				record.IsUnique =
					(Connection.GetScalar<int>(string.Format(sqlColConstrains.ToString(), tableName, record.Name, "U")) > 0);
		...
		private void Init()
		{
			...
			sqlProcedures.Append(
				"SELECT * FROM ALL_PROCEDURES WHERE OBJECT_TYPE = \'PROCEDURE\'  OR  OBJECT_TYPE = \'FUNCTION\' \n");
