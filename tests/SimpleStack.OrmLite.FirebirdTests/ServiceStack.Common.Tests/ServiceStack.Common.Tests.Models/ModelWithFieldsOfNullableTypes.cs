...
namespace SimpleStack.Common.Tests.Models
{
	...
	public class ModelWithFieldsOfNullableTypes : IHasIntId, IHasId<int>
	{
		...
		public static void AssertIsEqual(ModelWithFieldsOfNullableTypes actual, ModelWithFieldsOfNullableTypes expected)
		{
			...
			{
				ModelWithFieldsOfNullableTypes.Log.Error(
					"Trouble with DateTime precisions, trying Assert again with rounding to seconds", ex);
				Assert.That(DateTimeExtensions.RoundToSecond(actual.NDateTime.Value.ToUniversalTime()),
				            Is.EqualTo(DateTimeExtensions.RoundToSecond(expected.NDateTime.Value.ToUniversalTime())));
			}
			try
			{
			...
			{
				ModelWithFieldsOfNullableTypes.Log.Error(
					"Trouble with float precisions, trying Assert again with rounding to 10 decimals", ex2);
			...
			{
				ModelWithFieldsOfNullableTypes.Log.Error(
					"Trouble with double precisions, trying Assert again with rounding to 10 decimals", ex3);
		...
		public static ModelWithFieldsOfNullableTypes CreateConstant(int id)
		{
			...
			modelWithFieldsOfNullableType1.NFloat = new float?(1.11f + (float) id);
			modelWithFieldsOfNullableType1.NDouble = new double?(1.11 + (double) id);
			modelWithFieldsOfNullableType1.NGuid =
				new Guid?(new Guid((id%240 + 16).ToString("X") + "7DA519-73B6-4525-84BA-B57673B2360D"));
			modelWithFieldsOfNullableType1.NLongId = new long?((long) 999 + id);
			modelWithFieldsOfNullableType1.NDecimal = new decimal?(id + 0.5m);
			modelWithFieldsOfNullableType1.NTimeSpan = new TimeSpan?(TimeSpan.FromSeconds((double) id));
